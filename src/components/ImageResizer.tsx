...
import defaultImgSrc from "../assets/seam_carving_example.jpg";
...
const ImageResizer = (props: ImageResizerProps): React.ReactElement => {
  const { withSeam = false, withEnergyMap = false } = props;

  const [imgAuthor, setImgAuthor] = useState<string | null>("Mr. Reid");
  const [imgAuthorURL, setImgAuthorURL] = useState<string | null>(
    "http://wordpress.mrreid.org/2010/10/18/seam-carving-for-content-aware-image-resizing/"
  );
  ...
  const [originalImgSize, setOriginalImgSize] = useState<ImageSize | null>(
    null
  );
  const [originalImgViewSize, setOriginalImgViewSize] =
  ...
  const [toWidthScaleString, setToWidthScaleString] = useState<
    string | undefined
  >(`${defaultWidthScale}`);
  const [toHeightScale, setToHeightScale] =
    useState<number>(defaultHeightScale);
  const [toHeightScaleString, setToHeightScaleString] = useState<
    string | undefined
  >(`${defaultHeightScale}`);
  ...
  const onWidthSizeChange = (size: string | undefined): void => {
    ...
    const scale = Math.max(
      Math.min(parseInt(size || "0", radix), maxScale),
      minScale
    );
    if (size) {
  ...
  const applyMask = (img: ImageData): void => {
    ...
    for (let y = 0; y < img.height; y += 1) {
      for (let x = 0; x < img.width; x += 1) {
        ...
        const [mR, mG, mB, mA] = getPixel(maskImgData, imgXYtoMaskXY({ x, y }));
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  const imgAuthorLink =
    imgAuthor && imgAuthorURL ? (
      <div className="text-xs text-gray-500 mt-2 flex justify-center items-center font-light">
  ...
  const originalImageSizeText = originalImgSize ? (
    <sup className="text-xs text-gray-500 whitespace-nowrap">
  ...
  const mask = originalImgViewSize ? (
    <div
      ...
      style={{ marginTop: `-${originalImgViewSize.h}px` }}>
      <Mask
      ...
      <div
        ...
        style={{ marginTop: "-36px", zIndex: 100 }}>
        <div className="mr-1">{maskControls}</div>
      </div>
    </div>
  ) : null;
    ...
    <FadeIn>
      <div className="flex flex-row justify-center items-center mb-1 sm:mb-0">
        <div className="flex-1 sm:flex sm:flex-row sm:items-center">
          ...
          <div className="text-xs text-gray-500 flex flex-row items-center justify-self-end">
            <div className="mr-1">
  ...
  const workingImageScrollableText =
    workingImgSize?.w &&
    originalImgViewSize?.w &&
    workingImgSize.w > originalImgViewSize.w ? (
      <span className="text-xs text-gray-500 ml-4">↔︎ scrollable</span>
    ) : null;
  ...
  const workingImage = (
    <FadeIn className={`mb-6 ${resizedImgSrc || !energyMap ? "hidden" : ""}`}>
      <div>
        <b>Resized image</b> {workingImageSizeText} {workingImageScrollableText}
      </div>
  ...
  const resultImage =
    workingImgSize && resizedImgSrc ? (
      <FadeIn className="mb-6">
        <div>
          <b>Resized image</b> {resultImageSizeText}
        </div>
        <img
          src={resizedImgSrc}
          width={workingImgSize.w}
        ...
        <div className="mb-2">
          <Checkbox disabled={isResizing} onChange={onUseOriginalSizeChange}>
            <span className="text-xs">
              Higher quality{" "}
              <span className="text-gray-500">(takes longer)</span>
            </span>
          </Checkbox>
        </div>
      </div>
